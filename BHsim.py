...
def get_system_state(time, black_holes):
    ...
    ypos = []
    for bi in black_holes:
        xyzpos = bi.position.value_in(units.parsec)
        xpos.append(xyzpos[0])
        ypos.append(xyzpos[1])
        #print(xyzpos[0])
...
def plot_orbit(x, y):
    ...
    for i in range(len(x)):
        xarr_plot.append(x[i][0:nParticlesToPlot])
        yarr_plot.append(y[i][0:nParticlesToPlot])
    ##
    
    pyplot.xlabel("t [Myr]")
    pyplot.ylabel("Y [pc]")
    ##
    pyplot.plot(xarr_plot, yarr_plot)
    #pyplot.plot(yarr_plot)
    ##
    #pyplot.plot(x, y, lw=1)
    pyplot.scatter(x[0], y[0], lw=0.1)
    pyplot.show()


def save_sim_data(obj):
...
def main(t_end, M_cl, R_Pl, dt_out, dt_bridge):
    ...
    #N = 100
    black_holes = new_plummer_gas_model(4, convert_nbody=converter)
    black_holes.mass = 1000000 | units.MSun
...
def new_option_parser():
    ...
    result = OptionParser()
    result.add_option("--dt_out", dest="dt_out", type="float", default = 0.001|units.Myr,

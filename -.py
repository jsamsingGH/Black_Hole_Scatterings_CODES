...
outputinfo_screenfiles  = 1 #options: 0,1,2
...
nbody_params_arr_1_INT  = np.array([0, use_25PN, 1, 1000000000, 10, outputinfo_screenfiles, 3, de_analysis,0,0], dtype='i')
...
#------------------------------------------------------------------------------------------  
    ...
    #---------------------------------------------------------------------
        ...
        r_simsurf_abin      = 20.0                  #unit of SMA
        #---------------------
        ...
        if (outputinfo_screenfiles == 1):
            ...
            fig1.savefig('time_ecc.pdf',    bbox_inches='tight')        
            fig2.savefig('time_rdist.pdf',  bbox_inches='tight')        
            ...
            plt.savefig('testorbits.eps', bbox_inches='tight')        
            plt.show()
...
#CHECK ALL THINGS BELOW INCL. THE NEW PLOT IN FIG. 1.
...
#---------------------------------------------------------------------
    ...
    if (pc == 0):
        ecc_plot_colors = ['orange', 'red', 'blue', 'green']
        pd_psymsize     = 4
        pd_linewidth    = 1.5
        pd_fillstyle    = 'none'
    if (pc == 1):
    ...
    for sc_e in range(0,nr_e):
        ...
        #--------------------
        #Analytical:
        f1ax1.plot(rp_parr, abs(de_ana)/scalefac_y, linestyle=':', color='grey')
        #Simulation:
        #X>0 (GT0):
        pos = pos_deGT0
        f1ax1.plot(rp_parr[pos], abs(de_sim[pos])/scalefac_y, linestyle='-', linewidth=pd_linewidth, color=ecc_plot_colors[sc_e])
        pos = pos_LzGT0
        f1ax1.plot(rp_parr[pos], abs(de_sim[pos])/scalefac_y, marker='^', linestyle='', markersize=pd_psymsize, markeredgewidth=1.0, fillstyle = 'none', markeredgecolor = ecc_plot_colors[sc_e], color=ecc_plot_colors[sc_e])
        #X<0 (LT0):
        pos = pos_deLT0
        f1ax1.plot(rp_parr[pos], abs(de_sim[pos])/scalefac_y, linestyle='--', linewidth=pd_linewidth, color=ecc_plot_colors[sc_e])
        pos = pos_LzLT0
        f1ax1.plot(rp_parr[pos], abs(de_sim[pos])/scalefac_y, marker='v', linestyle='', markersize=pd_psymsize, markeredgewidth=1.0, fillstyle = 'full', markeredgecolor = ecc_plot_colors[sc_e], color=ecc_plot_colors[sc_e])
        #--------------------
        #Figure 2:
        #--------------------
        efin    = eini + de_sim
        f1ax2.plot(rp_parr, efin, linestyle='-', linewidth=pd_linewidth, color=ecc_plot_colors[sc_e])
        f1ax2.plot(rp_parr, efin, marker='o', linestyle='', markersize=pd_psymsize, markeredgewidth=1.0, fillstyle = pd_fillstyle, markeredgecolor = ecc_plot_colors[sc_e], color=ecc_plot_colors[sc_e])
        #incl guide lines:
        ...
        f1ax3.plot(rp_parr, tGW_fin_arr/tGW_ini_arr, marker='x', linestyle='', markersize=pd_psymsize, markeredgewidth=1.0, fillstyle = pd_fillstyle, markeredgecolor = ecc_plot_colors[sc_e], color=ecc_plot_colors[sc_e])
        
        esid_index  = 1 #(survive (10))   
        tGW_ini_arr = analyze_simexp1_data_arr[sc_e,:,0, 3]
        tGW_fin_arr = analyze_simexp1_data_arr[sc_e,:,esid_index, 4]
        f1ax3.plot(rp_parr, tGW_fin_arr/tGW_ini_arr, linestyle=p2_linestyle, linewidth=1.5, color=ecc_plot_colors[sc_e])
        f1ax3.plot(rp_parr, tGW_fin_arr/tGW_ini_arr, marker='s', linestyle='', markersize=pd_psymsize, markeredgewidth=1.0, fillstyle = pd_fillstyle, markeredgecolor = ecc_plot_colors[sc_e], color=ecc_plot_colors[sc_e])
        #--------------------
...
f1ax1.set_xlabel(r'$r_{\rm p}/a$')
f1ax1.set_ylabel(r'$|{\delta}e| = |e_{\rm fin} - e_{\rm ini}|$')
...
f1ax3.set_xlabel(r'$r_{\rm p}/a$')
f1ax3.set_ylabel(r'$\tau_{\rm fin}/\tau_{\rm ini}$')



#save figure:
plt.savefig('de_dt_fig1.eps', bbox_inches='tight')        
